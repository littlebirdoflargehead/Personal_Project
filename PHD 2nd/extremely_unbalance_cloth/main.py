...
def train(Config):
    ...
    Good_DataLoader = DataLoader(dataset=Good_Dataset, batch_size=Config.batch_size, shuffle=True,
                                 num_workers=Config.num_workers)
    Bad_DataLoader = DataLoader(dataset=Bad_Dataset, batch_size=Config.batch_size, shuffle=True,
                                num_workers=Config.num_workers)
    ...
    for epoch in range(Config.max_epoch):
        for i, images_g in enumerate(Good_DataLoader):
            ...
            Loss = loss_dif + 10 * torch.pow(torch.mean(loss_g) - 21952.0723, 2) + torch.var(loss_b) + torch.var(
                loss_g)
        ...
        if epoch % 8 == 7:
            ...
            accuracy_list, confusion_matrix_list = test(Config, model, threshold)
...
def test(Config, model=None, threshold=[150]):
    ...
    # step1: 数据
    Good_Dataset_test = GoodOrBadCloth(root=Config.train_data_root, good=True, train=False)
    Bad_Dataset_test = GoodOrBadCloth(root=Config.train_data_root, good=False, train=False)
    ...
    for dataset_idx in range(len(DataLoaders)):
        for i, images in enumerate(DataLoaders[dataset_idx]):
            ...
            for k in range(n):
                ...
                loss = VAE_Loss(images, re_images_mean, re_img_logvar, z_mean, z_logvar)
                elbo = elbo + loss / n

            # 统计样本是否被正确分类
            for j in range(len(threshold)):
                if len(confusion_matrix_list)>j:
                    confusion_matrix = confusion_matrix_list[j]
                else:
                    confusion_matrix = np.zeros([2, 2])
                    confusion_matrix_list.append(confusion_matrix)
                confusion_matrix[dataset_idx, 0] += torch.sum(elbo<threshold[j])
                confusion_matrix[dataset_idx, 1] += torch.sum(elbo>=threshold[j])
...
Config._parse({'load_model_path': 'checkpoints/vae-190920_17:50:35.pth'})  # 训练阶段2 # tensor z / with sigmoid / determined logvar
