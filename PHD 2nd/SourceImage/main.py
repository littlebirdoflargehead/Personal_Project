...
from utils import SparseAdam
from data import simulated_signal_dataset
from model import GCN_d, GCN_ADJ
...
def train():
    # GCN模型
    # gcn = GCN_d(k=k, Layers=[128, 128, 64, 32, 16], Cortex=Cortex, Gain=Gain, device=device)
    gcn = GCN_ADJ(k=k, Layers=[16, 32, 64, 32, 16], Cortex=Cortex, Gain=Gain, perlayer=True, device=device)
    # gcnd.load(os.path.join('checkpoints', 'gcn-200513_18:55:07.pth'))
    # 数据集(训练集与测试集)
    ...
    dataset_test = simulated_signal_dataset(path='data', gain=Gain, device=device,
                                            filters=['.pkl', 'test', '_nPatches{}'.format(nPatches),
                                                     '_extent{}'.format(extent)])
    # 优化器
    # optimizer = SparseAdam(gcn.parameters())
    optimizer = torch.optim.Adam(gcn.parameters(), weight_decay=1e-4)
    # optimizer.load_state_dict(torch.load(os.path.join('checkpoints', 'adam-200513_18:55:07.pth')))
    # 训练结果写入csv
    ...
    for epoch in range(max_epoch):
        ...
        while True:
            ...
            s_proj_tensor = gcn(B_proj_tensor)
            ...
            loss = torch.mean((B_proj_tensor - B_reproj_tensor).pow(2)) + torch.sum(
                (L_svd.view(-1, 1, 1) * s_).pow(2))  # torch.mean(L_tensor.bmm(s_).pow(2))
        ...
        if (epoch + 1) % checkpoint_fre == 0:
            ...
            ratio_tensor, B_tensor, W_tensor, TBFs_tensor = dataset_test.np_to_torch(ratio_array, B_array, W_array,
                                                                                     TBFs_array)
            L_tensor = torch.matmul(W_tensor, dataset_test.gain)
            B_proj_tensor = B_tensor.bmm(TBFs_tensor.transpose(1, 2))
            s_proj_tensor = gcn(B_proj_tensor)
            B_reproj_tensor = L_tensor.bmm(s_proj_tensor)
            loss = torch.mean((B_proj_tensor - B_reproj_tensor).pow(2))
            s_est = s_proj_tensor.bmm(TBFs_tensor).detach().cpu().numpy().astype(np.float64) * ratio_array.reshape(
                -1, 1, 1)
            for i in range(len(ActiveVox_List)):
