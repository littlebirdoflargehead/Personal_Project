...
def pinv(A,reg):
    ...
    # A_inv = np.mat(reg * np.eye(A.shape[1]) + A.T.dot(A)).I.dot(A.T)
    return A_inv

def pinv_Gauss(A,reg):

    A_inv = np.linalg.solve(reg * np.eye(A.shape[1]) + A.T.dot(A), A.T)
    # A_inv = np.mat(reg * np.eye(A.shape[1]) + A.T.dot(A)).I.dot(A.T)
    return A_inv

def pinv_origin(A,reg):
    # np.mat(reg * np.eye(A.shape[1]) + A.T.dot(A)).I.dot(A.T)
...
#        WeightOfEachWindow([],[],i) = weightOfEachWindow; #存储每个窗口的权重系数
    ...
    pinvOfInput_origin = pinv_origin(InputOfOutputLayer,c)
    pinvOfInput_Gauss = pinv_Gauss(InputOfOutputLayer,c)
...
#        weightOfEnhanceLayerAdd = weightOfEnhanceLayer[:,N3+e*M:N3+(e+1)*M]
        ...
        D = pinvOfInput_origin.dot(OutputOfEnhanceLayerAdd)
        C = OutputOfEnhanceLayerAdd - InputOfOutputLayer.dot(D)
        if C.all() == 0:
            w = D.shape[1]
            B_origin = np.mat(np.eye(w) - np.dot(D.T,D)).I.dot(np.dot(D.T,pinvOfInput_origin))
        else:
            B_origin = pinv_origin(C,c)
        pinvOfInput_origin = np.vstack([(pinvOfInput_origin - D.dot(B_origin)), B_origin])

        D = pinvOfInput_Gauss.dot(OutputOfEnhanceLayerAdd)
        C = OutputOfEnhanceLayerAdd - InputOfOutputLayer.dot(D)
        if C.all() == 0:
            w = D.shape[1]
            B_Gauss = np.mat(np.eye(w) - np.dot(D.T, D)).I.dot(np.dot(D.T, pinvOfInput_Gauss))
        else:
            B_Gauss = pinv_Gauss(C, c)
        pinvOfInput_Gauss = np.vstack([(pinvOfInput_Gauss - D.dot(B_Gauss)), B_Gauss])


        pinvOfInput_real = pinv(tempOfLastLayerInput, c)
        print('Epoch %d 修改前' % (e + 1), np.max(pinvOfInput_origin - pinvOfInput_real))
        print('Epoch %d 高斯消元' % (e + 1), np.max(pinvOfInput_Gauss - pinvOfInput_real))
        print('Epoch %d QR分解' % (e + 1), np.max(pinvOfInput - pinvOfInput_real))




        OutputWeightEnd = pinvOfInput.dot(train_y)
        OutputWeightEnd_origin = pinvOfInput_origin.dot(train_y)
        OutputWeightEnd_Gauss = pinvOfInput_Gauss.dot(train_y)
        ...
        OutputOfTrain1_origin = InputOfOutputLayer.dot(OutputWeightEnd_origin)
        TrainingAccuracy_origin = show_accuracy(OutputOfTrain1_origin, train_y)

        OutputOfTrain1_Gauss = InputOfOutputLayer.dot(OutputWeightEnd_Gauss)
        TrainingAccuracy_Gauss = show_accuracy(OutputOfTrain1_Gauss, train_y)

        train_acc[0][e+1] = TrainingAccuracy
        print('Epoch %d 修改前 Incremental Training Accuracy is :'% (e + 1), TrainingAccuracy_origin * 100, ' %' )
        print('Epoch %d 高斯消元 Incremental Training Accuracy is :' % (e + 1), TrainingAccuracy_Gauss * 100, ' %')
        print('Epoch %d QR分解 Incremental Training Accuracy is :'% (e + 1), TrainingAccuracy * 100, ' %')
        ...
        OutputOfTest1_origin = InputOfOutputLayerTest.dot(OutputWeightEnd_origin)
        TestingAcc_origin = show_accuracy(OutputOfTest1_origin, test_y)

        OutputOfTest1_Gauss = InputOfOutputLayerTest.dot(OutputWeightEnd_Gauss)
        TestingAcc_Gauss = show_accuracy(OutputOfTest1_Gauss, test_y)
        
        Test_time = time.time() - time_start
        test_time[0][e+1] = Test_time
        test_acc[0][e+1] = TestingAcc
        print('Epoch %d 修改前 Incremental Testing Accuracy is : '% (e + 1), TestingAcc_origin * 100, ' %' )
        print('Epoch %d 高斯消元 Incremental Testing Accuracy is : ' % (e + 1), TestingAcc_Gauss * 100, ' %')
        print('Epoch %d QR分解 Incremental Testing Accuracy is : '% (e + 1), TestingAcc * 100, ' %' )
