...
def test_individul(prefix='Set5',scale=4,mode='rgb',normalized=True):
    ...
    for file in os.listdir(pic_path):
        ...
        print('%s_PSNR/SSIM:  bicubic    %0.2f/%0.2f ; SRDenseNet   %0.2f/%0.2f' % (file[0:len(file) - 4], PSNR_bicubic,SSIM_bicubic,PSNR_generated,SSIM_generated))
        ...
        bicubic_img_path = base_test_images + prefix + '/' + prefix + "_%s_bicubic_%0.2f|%0.2f.png" % (file[0:len(file)-4],PSNR_bicubic,SSIM_bicubic)
        generated_path = base_test_images + prefix + '/' + prefix + "_%s_generated_%0.2f|%0.2f.png" % (file[0:len(file)-4],PSNR_generated,SSIM_generated)
    ...
    print('Average_PSNR/SSIM:  bicubic    %0.2f/%0.2f ; VDSR_new   %0.2f/%0.2f' % (
...
class SRModel:
    ...
    def build_model(self, load_weights=False,) -> Model:
        '创建SRDense_net网络模型对象'
        SRDense_net = SRDenseNet_H(self.img_width, self.img_height, self.batch_size, self.channels)
    ...
    def train_model(self, image_dir, nb_images=50000, epochs=1, normalized=True):
        ...
        for i in range(epochs):
            ...
            self.model.save_weights(base_weights_path + "SRDenseNet_Epoch%d_weights%0.2f.h5" % (i, total_psnr/nb_images), overwrite=True)
...
# SRDenseNet_Test.build_model(load_weights=True)
# SRDenseNet_Test.train_model(train_path, nb_images=50000, epochs=10, normalized=True)
