...
def test_individul(prefix='Set5',scale=4,mode='rgb',normalized=True):
    ...
    for file in os.listdir(pic_path):
        ...
        if mode=='ycrcb':
            ...
            lr_test = np.empty((1,bicubic_ycbcr.shape[0],bicubic_ycbcr.shape[1],1))
            lr_test[0,:,:,0] = bicubic_ycbcr[:,:,0]
        elif mode=='rgb':
            lr_test = np.empty((1, bicubic_rgb.shape[0], bicubic_rgb.shape[1], bicubic_rgb.shape[2]))
    ...
    print('Average_PSNR/SSIM:  bicubic    %0.2f/%0.2f ; VDSR_new   %0.2f/%0.2f' % ( total_psnr_bicubic/l, total_ssim_bicubic/l, total_psnr_generated/l, total_ssim_generated/l))
...
class SRModel:
    ...
    def __init__(self, img_width=96, img_height=96, batch_size=16, mode='ycrcb'):
        ...
        self.weights_path = base_weights_path + "VDSR_net_weights.h5"


    def build_model(self, load_weights=False,) -> Model:
        '创建VDSR网络模型对象'
        VDSR_net = VDSR(self.img_width, self.img_height, self.batch_size, self.channels)
        ip = Input(shape=(self.img_width, self.img_height, self.channels), name='x_generator')

        self.model = VDSR_net.create_sr_model(ip)
        self.model.summary()
        self.model.compile(Adam(lr=0.01), loss='mse', metrics=[PSNRLoss])
    ...
    def train_model(self, image_dir, nb_images=50000, epochs=1, normalized=True):
        ...
        for i in range(epochs):
            ...
            for real_rgb in datagen.flow_from_directory(image_dir, class_mode=None, batch_size=self.batch_size,
                ...
                try:
                    ...
                    for j in range(self.batch_size):
                        ...
                        bicubic_rgb = imresize(lr_rgb, size=scale*100, interp='bicubic')
                        if self.mode=='ycrcb':
                            real_ycrcb = rgb2ycbcr(real_rgb[j])
                            bicubic_ycrcb = rgb2ycbcr(bicubic_rgb)
                            '只取ycbcr中的y通道进行训练'
                            lr_train[j,:,:,0] = bicubic_ycrcb[:,:,0]
                            sr_train[j,:,:,0] = real_ycrcb[:,:,0]
                        elif self.mode=='rgb':
                            lr_train[j,:,:,:] = bicubic_rgb
            ...
            self.model.save_weights(base_weights_path + "VDSR_new_Epoch%d_weights%0.2f.h5" % (i, total_psnr/nb_images), overwrite=True)
...
#
# SRDenseNet_Test = SRModel(img_width=img_width, img_height=img_height, batch_size=10, mode = 'ycrcb')
# SRDenseNet_Test.build_model(load_weights=False)
# SRDenseNet_Test.train_model(train_path, nb_images=50000, epochs=2, normalized=True)
...
# 预测时使用更大的输入层(使用原图)
test_individul(prefix='Set5',scale=5, mode = 'rgb', normalized=False)
test_individul(prefix='Set14',scale=5,mode = 'rgb', normalized=False)